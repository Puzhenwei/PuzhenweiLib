...
public class FileUtils {
    ...
    /**
     * Opens a {@link FileInputStream} for the specified file, providing better error messages than simply
    ...
    /**
     * Opens a {@link FileOutputStream} for the specified file, checking and creating the parent directory if it
     * does not exist.
     * <p/>
     * At the end of the method either the stream will be successfully opened, or an exception will have been thrown.
     * <p/>
     ...
     * the UTF-8 character set. This function is primarily intended for usage with {@link URL} which
     ...
     * modified date/times using {@link File#setLastModified(long)}, however it is not guaranteed that the
     ...
     * {@link File#setLastModified(long)}, however it is not guaranteed that those operations will succeed. If
     ...
     * modified date/times using {@link File#setLastModified(long)}, however it is not guaranteed that those
     ...
     * {@link #copyURLToFile(URL, File, int, int)} with reasonable timeouts to prevent this.
     *
    ...
    /**
     * Copies bytes from an {@link InputStream} <code>source</code> to a file <code>destination</code>. The
     ...
     * <p/>
     * This method repeatedly tests {@link File#exists()} until it returns true up to the maximum time specified
     * in seconds.
     *
     ...
     * @throws IOException in case of an I/O error
     * @throws java.nio.charset.UnsupportedCharsetException thrown instead of
     *             {@link UnsupportedEncodingException} in version 2.2 if the encoding is not supported by the
     *             VM
     * @since 2.1
     */
    ...
    /**
     * Returns the size of the specified file or directory. If the provided {@link File} is a regular file, then
